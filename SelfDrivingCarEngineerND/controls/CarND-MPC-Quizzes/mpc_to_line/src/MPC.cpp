...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
    //part of cost that depends on the reference state
    for (int t = 0; t < N; t++) {
      fg[0] += CppAD::pow(vars[cte_start + t], 2); //Minimize cross track error
      fg[0] += CppAD::pow(vars[epsi_start + t], 2); //Minimize steering error
      fg[0] += CppAD::pow(vars[v_start + t] - ref_v, 2); //Minimize velocity error
    }

    // Minimize the use of actuators.
    //smooth the angular and linear acceleration
    for (int t = 0; t < N - 1; t++) {
    ...
    //Next control input should be just as smooth as the previous one
    for (int t = 0; t < N - 2; t++) {
    ...
    fg[1 + cte_start] = vars[cte_start]; //cross track error
    fg[1 + epsi_start] = vars[epsi_start]; //heading error
    ...
    for (int t = 1; t < N; ++t) {
      ...
      AD<double> f0 = coeffs[0] + coeffs[1] * x0; //f(x)
      AD<double> psides0 = CppAD::atan(coeffs[1]); //desired psi value
