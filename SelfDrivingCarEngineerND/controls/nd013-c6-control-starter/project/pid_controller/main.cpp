...
double normalizeAngle(double angle){
  if (abs(angle) > M_PI){
    ...
    if (angle < -M_PI){
      angle += 2*M_PI;
      std::cout << "  ADD 2PI" << std::endl;

    }
    if (angle > M_PI){
      angle -= 2*M_PI;
      std::cout << "  MINUS 2PI" << std::endl;
    }
  }
  return angle;
}
...
//   pid_steer.Init(   .225, .001, 0.15,   1.2, -1.2);
//   pid_throttle.Init(.1, .005, .1,    1, -1);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          ///////////////////////////////////////
          //D: Use desired yaw from trajectory
          ///////////////////////////////////////
          int min_dist_idx = 0;
          double min_dist = std::numeric_limits<double>::max();

          for (int j = 0; j < x_points.size(); j++){
            min_dist = std::hypot(x_points[min_dist_idx] - x_position, y_points[min_dist_idx] - y_position);
          ...
          //Get angle between current position and closest point to car on the trajectory
          des_yaw = angle_between_points(x_position, y_position, x_points[min_dist_idx], y_points[min_dist_idx]);

          //Error = current yaw - desired yaw (pointing to trajectory)
          error_steer = normalizeAngle(des_yaw - yaw) ;

          cout  << "error_steer(" << error_steer << ") =" 
                << "des_yaw(" << des_yaw << ") - yaw(" << yaw << ")" << std::endl;

          /**
          * TODO (step 3): uncomment these lines
          **/
          // Compute control to apply
          pid_steer.UpdateError(error_steer, true);
          ...
          **/
          ...
          double des_velocity = v_points[min_dist_idx];
          //Error for throttle = Actual Velocity - velocity at start of trajectory
          error_throttle = des_velocity - velocity;
          cout << "error_throtle(" << error_throttle << ") =" << "des_vel(" << des_velocity << ") - actl_vel(" << velocity << ")" << std::endl;
          ...
          **/
          ...
          std::printf("(error_steer, error_throttle) = (%f, %f) \n", error_steer, error_throttle);
          std::printf("(X,Y,Yaw) = (%f, %f, %f) \n", x_position, y_position, yaw);
