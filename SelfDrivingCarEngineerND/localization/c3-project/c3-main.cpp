...
Eigen::Matrix4d NDT(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations){
	...
	Eigen::Matrix4d scan_tf = transform3D(
		...
		PointCloudT::Ptr scan_aligned (new PointCloudT);
		
		ndt.align(*scan_aligned, scan_tf.cast<float>());
	
		if (ndt.hasConverged()){
			std::printf("NDT has converged with fitness score of %f \n", ndt.getFitnessScore ());
			transformation_matrix = ndt.getFinalTransformation().cast<double>();
			return transformation_matrix;
		}
		else {
			std::printf("NDT did not converged with fitness score of %f \n", ndt.getFitnessScore ());
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            ndt.setTransformationEpsilon (0.0001); // Setting minimum transformation difference for termination condition.
            ndt.setResolution(1); //Setting Resolution of NDT grid structure (VoxelGridCovariance).
            ndt.setStepSize(1); // Setting maximum step size for More-Thuente line search.

          	Eigen::Matrix4d pose_tf = transform3D(pose.rotation.yaw, pose.rotation.pitch, pose.rotation.roll, pose.position.x, pose.position.y, pose.position.z);
          
          	pose_tf = NDT(ndt, cloudFiltered, pose, 5); 

			//pose = ....
			pose = getPose(pose_tf);

			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
          	PointCloudT::Ptr transformed_scan (new PointCloudT);
			pcl::transformPointCloud (*cloudFiltered, *transformed_scan, pose_tf);
