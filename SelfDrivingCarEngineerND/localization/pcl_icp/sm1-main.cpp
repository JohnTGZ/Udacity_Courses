...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
    ...
    //1. Tranform source to the startingPose
  
  
  	Eigen::Matrix4d startingpose_tf = transform3D(
    ...
    if (icp.hasConverged()){
      std::printf("ICP converged!");
      Eigen::Matrix4d icp_tf = icp.getFinalTransformation().cast<double>() * startingpose_tf;
      return icp_tf;
    }
    else {
    //6. If ICP did not converge, log the message and return original identity matrix
      std::printf("ICP did not converge! Returning identity matrix");
    }
	...
	while (!viewer->wasStopped())
  	{
		...
		if( matching != Off){
			if( matching == Icp)
				transform = ICP(mapCloud, cloudFiltered, pose, 15); //TODO: change the number of iterations to positive number
