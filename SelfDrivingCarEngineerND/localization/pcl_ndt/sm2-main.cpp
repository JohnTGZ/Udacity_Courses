...
Eigen::Matrix4d NDT(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations){
  ...
  Eigen::Matrix4d scan_tf = transform3D(
    ...
    ndt.setMaximumIterations (iterations);
  	...
  	if (ndt.hasConverged()){
    	std::printf("NDT has converged with fitness score of %f", ndt.getFitnessScore ());
//   	Eigen::Matrix4d final_tf = ndt.getFinalTransformation().cast<double>();
//      transformation_matrix = final_tf * scan_tf;
  	transformation_matrix = ndt.getFinalTransformation().cast<double>();
  	return transformation_matrix;
    }
  else {
    	std::printf("NDT did not converged with fitness score of %f", ndt.getFitnessScore ());
