...
//For each association pair the following is done.
...
vector<int> NN(PointCloudT::Ptr target, PointCloudT::Ptr source, Eigen::Matrix4d initTransform, double dist){
    ...
    pcl::transformPointCloud(*source, *source_transformed, initTransform);
	...
	for (std::size_t i = 0; i < source_transformed->size(); i++){
		vector<int> pointIdxRadiusSearch; //Index representing single point in 'target' point cloud
		vector<float> pointRadiusSquaredDistance; //Squared distance between associated source and target point
		if (kdtree.radiusSearch((*source_transformed)[i], dist, pointIdxRadiusSearch, pointRadiusSquaredDistance ) > 0){
			associations.push_back(pointIdxRadiusSearch[0]);
		} else{
			associations.push_back(-1); //push back invalid point
		}
	}
...
vector<Pair> PairPoints(vector<int> associations, PointCloudT::Ptr target, PointCloudT::Ptr source, bool render, pcl::visualization::PCLVisualizer::Ptr& viewer){
	...
	for (PointT point : source->points ){
		...
		if (i >= 0){ //check if index is valid
			...
			Pair pair = Pair(Point(point.x, point.y, 0), Point(association.x, association.y, 0));
...
void getSize(const std::string name, Eigen::MatrixXd mat){
	std::cout << "The matrix " << name << " is of size ("
...
Eigen::Matrix4d ICP(vector<int> associations, PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations, pcl::visualization::PCLVisualizer::Ptr& viewer){
	...
	Eigen::Matrix4d transform_startingpose = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, 
														startingPose.position.x, startingPose.position.y, startingPose.position.z);
	

	PointCloudT::Ptr source_transformed (new PointCloudT);
	pcl::transformPointCloud(*source, *source_transformed, transform_startingpose);
	 ...
	 */
  	...
  	// 1) create matrices P and Q which are both 2 x 1 and represent mean point of pairs 1 and pairs 2 respectivley.
	...
	std::vector<Pair> pair_points = PairPoints(associations, target, source_transformed, true, viewer);

	for (Pair pair : pair_points){
		P(0,0) += pair.p1.x;
		P(1,0) += pair.p1.y;
		Q(0,0) += pair.p2.x;
		Q(1,0) += pair.p2.y;
	...
	P(0,0) /= pair_points.size();
	P(1,0) /= pair_points.size();
	Q(0,0) /= pair_points.size();
	Q(1,0) /= pair_points.size();
	...
	Eigen::MatrixXd X(2, pair_points.size());
	Eigen::MatrixXd Y(2, pair_points.size());

	for (std::size_t i = 0; i < pair_points.size(); i++){
		X(0, i) = pair_points[i].p1.x - P(0,0);
		X(1, i) = pair_points[i].p1.y - P(1,0);

		Y(0, i) = pair_points[i].p2.x - Q(0,0);
		Y(1, i) = pair_points[i].p2.y - Q(1,0);
	}


	/**
	 * 3) Compute d x d covariance matrix
	 */
	...
	// Eigen::MatrixXd W(source->size(), source->size());
	// W.setIdentity();
	/**
	 * EQUATION: S = X * W * Y.T
	 */
	...
	Eigen::MatrixXd D(svd.matrixV().cols(), svd.matrixV().cols());
	D.setIdentity();

	D(svd.matrixV().cols()-1, svd.matrixV().cols()-1) = (svd.matrixV() * svd.matrixU().transpose()).determinant();

	/**
	 * 4) Compute SVD and then get rotation
	 */
	...
	transformation_matrix = transformation_matrix * transform_startingpose;
	...
	viewer->addText("Score", 200, 200, 32, 1.0, 1.0, 1.0, "score",0);
  	...
  	while (!viewer->wasStopped ())
  	{
		if(matching){
			...
			Eigen::Matrix4d transform = ICP(associations, target, source, pose, 10, viewer);
