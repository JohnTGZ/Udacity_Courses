...
def compute_precision_recall(det_performance_all, conf_thresh=0.5):
    ...
    # compute precision (Positive predictive value: When an object is detected, what are the chances of it being an actual object?)
    precision = (true_positives) / (true_positives + false_positives)
    
    # compute recall (True positive rate: What are the chances of an actual object being detected?)
...
def pcl_to_bev(lidar_pcl, configs, vis=False):
    ...
    print(f"max intensity:{np.amax(lidar_pcl_int[:, 3])}")
    print(f"min intensity:{np.amin(lidar_pcl_int[:, 3])}")
