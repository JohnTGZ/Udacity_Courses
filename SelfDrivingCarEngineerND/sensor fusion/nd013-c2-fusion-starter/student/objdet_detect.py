...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        configs.arch = 'fpn_resnet' #18 layers
        ...
        configs.pin_memory = True
        configs.distributed = False  # For testing on 1 GPU onlys
        ...
        if 'darknet' in configs.arch:
            ...
            for sample_i in range(len(output_post)):
                ...
                for obj in detection:
                    ...
                    detections.append([1, np.float(x), np.float(y), np.float(0.0), np.float(1.50), np.float(w), np.float(l), np.float(yaw)])    
