...
std::vector<bool> collision_checker(const State& ego_vehicle,
  ...
  for (size_t obs = 0; obs < obstacles.size(); ++obs) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      ego_circle_center.x = ego_vehicle.location.x + 
        					CIRCLE_OFFSETS[c] * std::cos(ego_vehicle.rotation.yaw);
      ego_circle_center.y = ego_vehicle.location.y + 
      ...
      for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
        ...
        double dist = std::sqrt(
          				std::pow((ego_circle_center.x - actor.location.x),2) +
          				std::pow((ego_circle_center.y - actor.location.y),2));
