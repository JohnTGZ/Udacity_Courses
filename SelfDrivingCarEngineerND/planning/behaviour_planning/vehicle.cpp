...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
   ...
   */
    ...
    std::vector<string> possible_successor_states = successor_states();

    vector<Vehicle> best_trajectory;
    double min_cost = std::numeric_limits<double>::max();

    std::string desired_state = "KL";
    //keep track of the total cost of each state.
    for (std::string state : possible_successor_states){
        vector<Vehicle> traj = generate_trajectory(state, predictions);
        
        // calculate the "cost" associated with that trajectory.
        double cost_for_state = calculate_cost(*this, predictions, traj);
