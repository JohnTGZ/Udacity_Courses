...
double HBF::heuristic(const double x, const double y, const vector<int>& goal){
    double dx = goal[0] - x;
    double dy = goal[1] - y;
...
vector<HBF::maze_s> HBF::expand(HBF::maze_s &state, const vector<int>& goal) {
  int g = state.g;
  double x = state.x;
  double y = state.y;
...
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start, 
  ...
  bool finished = false;
  while(!opened.empty()) {
    //sort opened by f-cost
    sort(opened.begin(), opened.end(), [](const maze_s& a, const maze_s& b){
        return a.f < b.f;
    });
